...
object NSphereCovering {
  ...
  def nSphereCovering(n: Int, alphaStep: Double, keepCubicShape: Boolean = false): Seq[Vector] = {
    val dim = n + 1
    if(n == 0) {
      Seq(Seq(-1.0), Seq(+1.0))
    } else {
      ...
      val cellVectors = (1 to (alphaMax / alphaStep).toInt).flatMap(i => {
        ...
        val border = sphere
          ...
          .map(splitting => (1/splitting.component.norm) *: splitting.fusion)
      ...
      val cubicNSphere = cellVectors.flatMap(v => {
        (0 to dimension(v)).flatMap(number => {
          val (vLeft, vRight) = v.splitAt(number)
