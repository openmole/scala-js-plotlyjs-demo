...
import org.openmole.plotlyjs.all.{axis, scatter}
import org.openmole.plotlyjs.{Layout, Plotly}
import plotlyjs.demo.demo.Demo
import plotlyjs.demo.labo.directions.restrictedspacetransformation.v4.IndexedTransformation
...
object DirectionsScatterDemo {
  ...
  private lazy val sc = sourcecode.Text {
    ...
    val dim = 3
    val results = Data.lowSphericalCorner(dim, 16) //new ParetoFront(dim, 128).front//Data.highSphericalCorner(dim, 4)//Data.simplexRST(dim, 16).toSeq//Data.lowInverseCorner(dim, 16, 2)//Data.lowSphericalCorner(dim, 16)//Data.dim8Sample100.map(_.take(dim))
    ...
    Plotly.newPlot(plotDiv.ref, plotDataSeq.toJSArray, layout._result)
