package plotlyjs.demo.labo.directions
...
import plotlyjs.demo.labo.directions.angularadjustment.{AngularAdjustment, CubicAngularAdjustment}
import plotlyjs.demo.labo.directions.angularadjustment.AngularAdjustment.Geometry
import plotlyjs.demo.labo.directions.buildingmethod.{BuildingMethod, BuildingMethodWithCache, BuildingMethodWithLines}
import plotlyjs.demo.labo.directions.restrictedspacetransformation.v4.Evaluation.{evaluationDiff, singleEvaluation}
import plotlyjs.demo.labo.directions.restrictedspacetransformation.v4.{Evaluation, IndexedTransformation}
  ...
  private lazy val sc = sourcecode.Text {
    ...
    div(
      ...
      reloadOnDemand("ParetoFront generation 2d", title => scatter3dDiv(
        title,
        ParetoFront.random(2, 128).map(_ :+ 0),
        Seq(0.0 at 3)
      )),
      reloadOnDemand("ParetoFront generation 3d", title => scatter3dDiv(
        title,
        ParetoFront.random(3, 128), //.map(_ :+ 0),
        Seq(0.0 at 3)
      )),
    )
  }
